...
class Session
{
    ...
    protected $valuesKey = 'cuveen.session';
    ...
    protected function generateDecoyCookie()
    {
        ...
        if ($this->decoy && !$has_decoy) {
            $this->put('decoy_value', md5(mt_rand()));
            setcookie(
                'PHPSESSID',
    ...
    public function start($restart = false)
    {
        ...
        if ($this->validateFingerprint()) {
            ...
            $this->checkLifespan();
            $this->put('session_loaded', date('U'));
            $this->put(
                'ttl',
                ($this->get('lifespan') - $this->get('session_loaded'))
            );
        }
    }
     ...
     * @return void
     */
    public function put($key, $value, $hash = false)
    {
        if ($hash) {
    ...
    {
        return (isset($_SESSION[$this->valuesKey][$key]))?true: false;
    }
